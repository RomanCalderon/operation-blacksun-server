...
public class CharacterMotor : MonoBehaviour
{
    ...
    private void UpdateFunction ()
    {
        ...
        Vector3 currentMovementOffset = velocity * Time.fixedDeltaTime;
        ...
        movement.velocity = ( transform.position - lastPosition ) / Time.fixedDeltaTime;
        ...
        if ( grounded && !IsGroundedTest () )
        {
            ...
            //Debug.Log ( Vector3.up * pushDownOffset );
            //Debug.Log ( $"pushDownOffset [{pushDownOffset}]" );

            transform.position += Vector3.up * pushDownOffset;
        }
        else // We were not grounded but just landed on something
        {
            if ( !grounded && IsGroundedTest () ) // Impacted ground
            {
                grounded = true;
